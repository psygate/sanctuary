 ...
 */
public class ToggleMessagesCommand extends NucleusPlayerCommand {
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] strings) throws Exception {
        ...
        boolean newstate = !state.isNotifyOnEnterExit();

        state.setNotifyOnEnterExit(newstate);
        NotificationListener.getInstance().persistPlayerSettings(state);

        player.sendMessage("Sanctuary messages now " + ((newstate) ? ChatColor.GREEN + "on" : ChatColor.RED + "off"));
    }

    @Override
    protected String[] getName() {
        return new String[]{"sanctuarytoggleentryexit"};
    }
}
