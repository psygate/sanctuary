 ...
 */
public class ForceHealSanctuary extends NucleusOPCommand {

    public ForceHealSanctuary() {
        super(0, 0);
    }

    @Override
    protected String[] getName() {
        return new String[]{"forceheal"};
    }
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] strings) throws Exception {
        SanctuaryManager.getInstance().getContaining(player.getLocation().getBlockX(), player.getLocation().getBlockZ(), player.getWorld().getUID())
                .forEach(sanctuary -> {
                    sanctuary.setHealth(sanctuary.getMaxHealth());
                    player.sendMessage("Force healed sanctuary: " + sanctuary);
                });
    }
}
